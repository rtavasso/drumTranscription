...
class BabySlakhDrums(PianoRollAudioDataset):
    SPLITS_FILENAME = 'splits.json'

    def __init__(self, path='data/babyslakh_drums', groups=None, sequence_length=None, seed=42, device=DEFAULT_DEVICE):
        self.splits = self._load_splits(path)
        default_groups = groups if groups is not None else list(self.splits.keys())
        super().__init__(path, default_groups, sequence_length, seed, device)
    ...
    def files(self, group):
        if group not in self.splits:
            raise ValueError(f'Unknown group {group}, available: {list(self.splits.keys())}')

        result = []
        for track_id in self.splits[group]:
            audio_path = self._find_audio_file(track_id)
            tsv_path = os.path.join(self.path, track_id + '.tsv')
            if not os.path.exists(tsv_path):
                raise FileNotFoundError(f'Annotation not found for {track_id} at {tsv_path}')
            result.append((audio_path, tsv_path))
        return result

    def _load_splits(self, path):
        splits_path = os.path.join(path, self.SPLITS_FILENAME)
        if not os.path.exists(splits_path):
            raise FileNotFoundError(f'Splits file not found at {splits_path}')
        with open(splits_path, 'r') as handle:
            return json.load(handle)

    def _find_audio_file(self, track_id):
        for extension in ('.flac', '.wav'):
            candidate = os.path.join(self.path, track_id + extension)
            if os.path.exists(candidate):
                return candidate
        raise FileNotFoundError(f'Audio file not found for {track_id} under {self.path}')
